...
function PageSlider(config) {
    ...
    this.banSlider=config.banSlider || false;//禁止滑动
    this.banLookNext=config.banLookNext||false;//静止向上滑动
    this.banLookPrev=config.banLookPrev||false;//静止向下滑动
    ...
    this.moveComplete=config.moveComplete || function(){
...
PageSlider.prototype = {
    ...
    _endHandle: function (e) {
        ...
        //触摸判断
        if (this.flag === this.swipe&&!this.banSlider) {
            if (this.swipe === 'X') {
                if (distanceX > 20) {
                    this.prev();
    ...
    moveTo:function(index,bo){
        ...
        var self=this;
        if(bo){
            self.pages[self.index] && Tool.removeClass(self.pages[self.index], self.toggleClass);
            self.pages[index] && Tool.addClass(self.pages[index], self.toggleClass);
            self.onComplete && self.onComplete.call(self);
            this.wraper.style.webkitTransition = '-webkit-transform ' + 0 + 's ' + this.animateFn;
        }else{
            //切换当前屏class
            this._toggleClassFn(this.pages, this.index, index);
            this.wraper.style.webkitTransition = '-webkit-transform ' + this.speed + 's ' + this.animateFn;
