 ...
 *
 */
class MaskObject extends egret.Sprite{
	public constructor() {
    	super();
	}
    ...
    private _sh;
    public sp;
	public init(source_name?:string,bo?:boolean){
...
//        shap.graphics.drawCircle(300,300,this.r);
    ...
    private vf = 0;
    private plength = 0;
    private min_w: number = 40;
    private min_h: number = 40;
    private nodeArr: Array<any> = [];
	public creatNode(){
    ...
    public maskView: egret.DisplayObjectContainer;
    public curs:egret.DisplayObject;
    public r: number = 40;
    public play(sx?:number,sy?:number) {
        //this.r = 60;
    ...
    public onenterframe(event: egret.Event): void {
        this.r+=8;
        ...
        if(this.nodeArr.length>0){
            let col = Math.floor((this.curs["offset"].stageX) / this.min_w);
            let row = Math.floor((this.curs["offset"].stageY) / this.min_h);
            //console.log("列：%d行：%d",col,row);
            let tag_node = this.nodeArr[col][row];
            ...
            let a: number = (this.vf / this.plength) * 100;
            //console.log(a);
            if(a >= 100) {
                this.dispose();
            }
    ...
    public dispose(){
        ...
        if(this.hasEventListener(egret.TouchEvent.TOUCH_BEGIN)){
            var s2:Scene2=<Scene2>this.parent;
